...
//! - **Pages** — Box answers `/items` a page at a time and says how many rows the folder
//!   holds. A folder wide enough produces pages for as long as anybody asks, so the offset is
//!   advanced a bounded number of times and no further.
...
//! unchanged — down to this file being `plugins/onedrive-crawler/src/walk.rs` with Box's
//! pagination in the comment: what a person gets back from pasting a folder should not depend
//! on which cloud it was in, and a limit reimplemented per provider is a limit that drifts.
...
pub struct Pending {
    /// The item id. The root the walk starts at is enqueued with its own, because Box names
    /// every folder by id — including the account root, which is `0`.
    pub id: String,
...
impl Walk {
    /// Starts at the folder the crawl was given, by the id Box names it with.
    #[must_use]
