...
//!
//! The canary page is not a recording. No FileJoker page has been measured at all, signed in or
...
const CANARY_PAGE: &str = r#"<html><head><title>FileJoker - Files</title></head>
...
fn homepage(body: &str) -> HttpResponse {
    response("https://filejoker.net/", body)
}
...
fn the_line_names_title_length_and_markers_and_nothing_else() {
    let line = unconfirmed_page_line("FileJoker", "the homepage", CANARY_PAGE);
    assert_no_canary(&line);
    assert!(
        line.starts_with("FileJoker: the homepage settled nothing"),
        "{line}"
    );
    assert!(line.contains("title \"FileJoker - Files\""), "{line}");
    assert!(
...
fn a_page_without_title_or_markers_says_so() {
    let line = unconfirmed_page_line("FileJoker", "the homepage", "<p>c4n4ry7f3a</p>");
    assert_no_canary(&line);
...
fn the_title_is_bounded_and_carries_no_control_characters() {
    ...
    let line = unconfirmed_page_line("FileJoker", "the homepage", &long);
    assert!(
...
async fn a_cookie_only_check_traces_the_unconfirmed_page_without_its_body() {
    let host = MockHost::new(vec![homepage(CANARY_PAGE)]);
    ...
    assert_eq!(
        ...
        ["https://filejoker.net/"],
        "the homepage: no FileJoker page is measured, so none replaces it"
    );
    let line = single_warn_line(&host);
    assert_no_canary(&line);
    assert!(line.contains("FileJoker - Files"), "{line}");
}

/// A page that settles the question has nothing to trace.
#[tokio::test]
async fn a_recognized_page_writes_nothing() {
    let host = MockHost::new(vec![homepage(
        r#"<title>FileJoker</title><a href="/?op=logout">Logout</a>"#,
    )]);
