...
//!
//! Mirrors `mcrapet/plowshare-modules-legacy`'s `filejoker.sh` step by step (there is no JD
//! plugin for FileJoker — see `crate::page`'s module doc): file page -> first form posted in free
//! mode -> captcha solved and countdown waited out -> second form posted -> direct link. Each
//! test drives the real resolver against queued mock responses and asserts what the plugin *did*
//! (which requests, which captcha, which wait), not just what it returned.
...
/// The file page's first-step form — plowshare's `F22`, with the fields it reads off it.
...
/// The answer to the first form — plowshare's `F1`, plus the countdown and a captcha.
...
<p>Please Wait <b>30</b> seconds</p>"#;

/// The final page carrying the direct link.
const LINK_PAGE: &str =
    r#"<a href="https://fs1.filejoker.net/d/tok3n/release.rar">Download File</a>"#;
...
async fn the_free_flow_solves_the_captcha_waits_and_posts_both_forms() {
    let host = MockHost::free(
        vec![
            ...
            html(DOWNLOAD2_PAGE),
            html(LINK_PAGE),
            file("https://fs1.filejoker.net/d/tok3n/release.rar"),
        ],
        Some("captcha-token"),
    );
    let resolver = FilejokerResolver::new(host.clone());
    ...
    assert_eq!(
        resolved.url.as_str(),
        "https://fs1.filejoker.net/d/tok3n/release.rar"
    );
    ...
    assert_eq!(
        ...
        Some("https://filejoker.net/")
    );

    // FileJoker's only credential is a cookie session; the free path must never ask for one.
    assert!(
        !*host.cookies_asked.lock().expect("lock"),
        "the free flow must resolve without a cookie session"
    );

    let requests = host.requests.lock().expect("lock");
    assert_eq!(requests.len(), 4, "page, download1, download2, transfer");
    // The first form is posted in free mode: the free marker survives, premium is gone.
    ...
    assert!(step_one.contains("usr_login="), "{step_one}");
    // The second form carries the captcha token.
    ...
    // The countdown was waited out, and the captcha was solved before it, so the token is as
    // fresh as possible when the form is posted.
    ...
    match &captchas[0] {
        CaptchaChallenge::RecaptchaV2(widget) => {
            ...
            assert!(widget.page_url.starts_with("https://filejoker.net/"));
        }
...
async fn a_hotlink_short_circuits_the_whole_flow() {
    let host = MockHost::free(
        vec![file("https://fs1.filejoker.net/d/tok3n/release.rar")],
        Some("unused"),
    );
    let resolver = FilejokerResolver::new(host.clone());
...
async fn a_stated_ip_limit_becomes_an_ip_block_without_solving_anything() {
    let host = MockHost::free(
        vec![html(
            "<div class=\"err\">Please wait 45 minutes, 30 seconds until the next download</div>",
        )],
        Some("unused"),
    );
    let resolver = FilejokerResolver::new(host.clone());
    ...
    assert_eq!(
        ...
        Some("filejoker.free_limit_reached")
    );
    assert_eq!(
...
async fn a_limit_without_a_duration_still_blocks_the_hoster() {
    let host = MockHost::free(
        vec![html("<p>Please wait until the next download</p>")],
        Some("unused"),
    );
    let resolver = FilejokerResolver::new(host);
...
/// A rejected captcha is retried once with a fresh challenge.
#[tokio::test]
async fn a_rejected_captcha_is_retried_once_then_reported() {
    ...
    assert_eq!(failure.code.as_deref(), Some("filejoker.captcha_rejected"));
    assert_eq!(
...
async fn a_page_without_a_form_reports_a_diagnosis() {
    let host = MockHost::free(vec![html("<title>Something else</title>")], Some("unused"));
    let resolver = FilejokerResolver::new(host);
    ...
    assert_eq!(failure.code.as_deref(), Some("filejoker.no_free_form"));
    assert!(
...
/// FileJoker's free tier refuses large files outright: no wait or captcha can get past it.
#[tokio::test]
async fn the_free_size_limit_is_reported_without_solving_anything() {
    let host = MockHost::free(
        vec![html(
    ...
    let failure = resolver
        .resolve(free_request())
        .await
        .expect_err("the free size limit must fail");

    assert_eq!(failure.category, FailureKind::AuthRequired);
    assert_eq!(failure.code.as_deref(), Some("filejoker.free_size_limit"));
    assert!(host.captchas.lock().expect("lock").is_empty());
}
