...
fn json(body: &str) -> Result<HttpResponse, Failure> {
    Ok(HttpResponse {
        status: 200,
        final_url: "https://katfile.biz/api/file/direct_link".to_owned(),
        headers: Vec::new(),
...
async fn a_direct_link_that_works_writes_nothing() {
    let host = MockHost::answering(json(
        r#"{"status":200,"msg":"OK","result":{"url":"https://dl.katfile.biz/d/abc123xyz/release.rar","size":"1024"}}"#,
    ));
