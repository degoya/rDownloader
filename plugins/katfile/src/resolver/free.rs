...
//! post `download2`, and take the direct link from what comes back. Every page is checked for an
//! IP limit first, because hitting one means no amount of waiting or captcha solving will help
//! until it expires. See `crate::page`'s module doc for the full IMPL-VERIFY record behind the
...
async fn free_transfer<H: PluginHost>(
    ...
    let page_response = host
        .http(range_probe(super::api::canonicalize_host(url)))
        .await?;
