//! Signing a Google Drive account in (RD-106-04).
//!
//! One of three siblings. This one holds no Drive logic at all: it obtains a token and hands it
//! to the vault, and `plugins/google-drive/` and `plugins/google-drive-crawler/` spend it
//! without ever seeing it. The `google_drive` provider row it fills belongs to the resolver,
...
//!   `{{secret:google_drive_access_token}}`, which the host expands on the way out.
