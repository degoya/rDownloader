//! The component: Premiumize's OAuth device flow, one call per step.
//!
...
const DEVICE_ENDPOINT: &str = "https://www.premiumize.me/token";
const TOKEN_ENDPOINT: &str = "https://www.premiumize.me/token";

/// Premiumize's device flow takes no scope; the token is the account's API key.
const RESPONSE_TYPE: &str = "device_code";
...
///
/// The code is a parameter now (RD-106-01). It used to be `premiumize_auth.flow_expired` for
...
fn refuse(code: &str, message: impl Into<String>, category: FailureKind) -> Failure {
    Failure {
        ...
        code: Some(format!("premiumize_auth.{code}")),
        params: Vec::new(),
    }
}
