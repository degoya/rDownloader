...
pub const CLIENT_ID: &str = "623166936";
...
/// language the person reads. `premiumize_auth` is this plugin's slug; the catalogue in `locales/`
...
mod tests {
    ...
    const STARTED: &str = r#"{
      "device_code": "abc123",
      "user_code": "WXYZ-1234",
      "verification_url": "https:\/\/www.premiumize.me\/webapp\/authorize",
      "expires_in": 600,
      "interval": 5
    }"#;
    ...
    fn a_device_code_answer_is_read_whole() {
        assert_eq!(
            ...
            Some(DeviceCode {
                ...
                verification_url: "https://www.premiumize.me/webapp/authorize".to_owned(),
                expires_in: Some(600),
                interval: Some(5),
            })
        );
    ...
    fn either_spelling_of_the_address_field_is_accepted() {
        ...
        assert_eq!(
            ...
            Some("https://www.premiumize.me/webapp/authorize".to_owned())
        );
    }
