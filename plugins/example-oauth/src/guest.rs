...
//!
//! What the `oauth` SDK template scaffolds, differing only in where it points: the endpoints are
//! on `oauth.example.invalid`, which resolves nowhere, and the contract tests answer for it.
...
const AUTHORIZE_ENDPOINT: &str = "https://oauth.example.invalid/oauth/authorize";
...
const DEVICE_ENDPOINT: &str = "https://oauth.example.invalid/oauth/device/code";
/// Where the code and the refresh material are exchanged for tokens.
const TOKEN_ENDPOINT: &str = "https://oauth.example.invalid/oauth/token";
...
const CLIENT_ID: &str = "example-oauth-client";
