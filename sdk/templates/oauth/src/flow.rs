...
/// language the person reads. `{{PLUGIN_SLUG}}` is this plugin's slug; the catalogue in
...
mod tests {
    ...
    fn an_unconfirmed_device_code_keeps_the_flow_open() {
        ...
        assert_eq!(
            read_token_answer(400, None, r#"{"error":"authorization_pending","interval":7}"#),
    ...
    fn a_device_authorization_answer_is_read_whole() {
        ...
        assert_eq!(
            read_device_code(r#"{"device_code":"","user_code":"WX","verification_uri":"https://x"}"#),
            None
        );
    }
